...
public class GuideView extends RelativeLayout {
	...
	private void init() {
		...
		//mPaint.setMaskFilter(new BlurMaskFilter(2, BlurMaskFilter.Blur.INNER));
			...
			for (HoleBean hole : mHoleList) {
				...
				switch (hole.getType()) {
					case HoleBean.TYPE_CIRCLE:
						mCanvas.drawCircle(rectF.centerX(), rectF.centerY(), hole.getRadius()+10, mPaint);
						break;
					case HoleBean.TYPE_RECTANGLE:
						mCanvas.drawRect(rectF, mPaint);
						break;
					case HoleBean.TYPE_ROUNDRECT:
						int radio = hole.getRoundRadius();
                        mCanvas.drawRoundRect( rectF, radio, radio, mPaint );
						break;
