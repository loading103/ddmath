...
//    }
     ...
     */
    public void setBadgeCount(Context context, int count, int iconResId) {
        if (count <= 0) {
            count = 0;
        } else {
...
//        }else if (Build.MANUFACTURER.toLowerCase().contains("LeMobile")) {//乐视
        ...
        }else if (Build.MANUFACTURER.toLowerCase().contains("huawei")||Build.BRAND.toLowerCase().equals("huawei")||Build.BRAND.toLowerCase().equals("honor")) {//华为
...
//            //Toast.makeText(context, "Not Found Support Launcher", Toast.LENGTH_LONG).show();
     ...
     * 重置Badge
     */
    public void resetBadgeCount(Context context, int iconResId) {
     ...
     */
    private void setBadgeOfVIVO(Context context,int count){
        try {
     ...
     */
    private void setBadgeOfOPPO(Context context,int count){
        try {
    ...
    private void setBadgeOfMadMode(Context context, int count, String packageName, String className) {
        Intent intent = new Intent("android.intent.action.BADGE_COUNT_UPDATE");
        intent.putExtra("badge_count", count);
