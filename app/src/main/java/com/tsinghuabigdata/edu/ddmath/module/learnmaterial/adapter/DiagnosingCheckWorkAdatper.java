...
public class DiagnosingCheckWorkAdatper extends BaseAdapter {
    ...
    public DiagnosingCheckWorkAdatper(Context context, List<SubmitQuestion> datas) {
        mContext = context;
        mDatas = datas;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            holder.diagnosingView = convertView.findViewById(R.id.tv_diagnosing_view);
        } else {
        ...
        if (item.getExerStatus() == SubmitQuestion.CORRECTED) {
            ...
            String data = item.getUsePrivilege()== DDWorkDetail.ST_BUYED ?"批阅中...":"等待批阅";
            if( item.getUploadType() == AppConst.UPLOAD_TYPE_MARKED ){
                data = "报告生成中";
            }
            holder.diagnosingView.setText( data );
        }
